...
%Lab image
ab = double(lab_img(:,:,2:3)); %matrix of all pixels in the image with values for a and b only
...
n_col = 3; %number of colors is equal to the number of centroids for kmenas; depends of image

rep = 5;
%repeat the clustering 'rep' times to avoid local minima
[cluster_idx, cluster_center] = kmeans(ab,n_col,'distance','sqEuclidean','Replicates',rep);
%labeling the pixels by clusters
pixel_labels = reshape(cluster_idx,nrows,ncols);
figure, imshow(pixel_labels,[]), title('image labeled by cluster index, repetitions = '+rep);

%separate image segments (by color)
